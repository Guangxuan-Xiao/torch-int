import torch
from torch_int._CUDA import bmm_s8t_s8n_s8t, bmm_s8t_s8n_s32t, bmm_s8t_s8n_f32t, baddmm_s8t_s8n_f32t
...
@torch.no_grad()
def test_baddmm_s8t_s8n_f32t():
    # used by q x k + mask
    B, M, K, N = 1, 512, 512, 32
    a = torch.randint(-128, 127, (B, M, K), dtype=torch.int8).cuda()
    b = torch.randint(-128, 127, (B, N, K), dtype=torch.int8).cuda()
    mask1 = torch.randn(M, N).cuda()
    mask2 = mask1.clone()
    alpha, beta = 0.001, 0.02
    c = baddmm_s8t_s8n_f32t(a, b, mask1, alpha, beta)
    # ic(c)
    c_gt = mask2.baddbmm(batch1=a.float(), batch2=b.float(
    ).transpose(1, 2), beta=beta, alpha=alpha)
    # ic(c_gt)
    ic(torch.mean((c_gt - c) ** 2))
...
if __name__ == '__main__':
    ...
    print('test_baddmm_s8t_s8n_f32t')
    test_baddmm_s8t_s8n_f32t()
