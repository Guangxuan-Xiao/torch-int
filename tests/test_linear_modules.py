import torch
from torch_int.nn.linear import W8A8B8O8LinearReLU, W8A8B8O8Linear, W8A8B32O32LinearWithScaling
...
def test_w8a8b32o32_linear_with_scaling():
    ...
    y_gt = linear(x)
    y_scale = y_gt.abs().max() / 127
    q_linear = W8A8B32O32LinearWithScaling.from_float(linear, x_scale, y_scale).cuda()
    q_y = q_linear(qx.cuda()).cpu()
    y_hat = q_y * y_scale
    r2 = (y_gt - y_hat).pow(2).mean() / y_gt.pow(2).mean()
    ic(r2)
