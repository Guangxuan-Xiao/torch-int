...
import torch
from torch_int.nn import Int8Linear, W8FakeA8Linear
...
def test_quant_linear_1():
    ...
    ic(linear.weight)
    int8_linear = Int8Linear.from_float(deepcopy(linear)).cuda()
    ...
    ic(dummy_input)
    y = linear(dummy_input.clone())
    ic(y)
    q_y = int8_linear(dummy_input.clone())
    ic(q_y)
    mse = (y - q_y).pow(2).mean()
    ic(mse)

    w8fakea8_linear = W8FakeA8Linear.from_float(deepcopy(linear)).cuda()
    q_y = w8fakea8_linear(dummy_input.clone())
    ic(q_y)
    mse = (y - q_y).pow(2).mean()
    ic(mse)
