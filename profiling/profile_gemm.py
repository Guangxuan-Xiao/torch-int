...
a, b = torch.randint(-127, 127, (L, C1),
                     device='cuda:0'), torch.randint(-127, 127, (C1, C2), device='cuda:0')
...
a, b = torch.randn(L, C1, device='cuda:0').half(
), torch.randn(C1, C2, device='cuda:0').half()
start = torch.cuda.Event(enable_timing=True)
end = torch.cuda.Event(enable_timing=True)
start.record()
torch.cuda.synchronize()
