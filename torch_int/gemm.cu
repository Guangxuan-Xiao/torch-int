...
torch::Tensor gemm(torch::Tensor A, torch::Tensor B, bool transa, bool transb)
 {
    ...
    torch::Tensor C = torch::zeros({n, m}, torch::dtype(torch::kInt).device(device));

    // Note that CuBLAS assumes column-major matrices, so we actually pass the transposes of A and B    
    stat = cublasGemmEx(handle, transb_, transa_, m, n, k, &alpha, B.data_ptr(), Btype, ldb, A.data_ptr(), Atype, lda, &beta, C.data_ptr(), Ctype, ldc, computeType, algo);
